...
bool CRAIDProcessor::ReadStripeUnit(unsigned long long StripeID, ///identifies the codeword (stripe)
    ...
    return m_pArray->m_pDisks[SymbolID + SubarrayID * m_Length].ReadData(
        StripeID * m_StripeUnitsPerSymbol + StripeUnitID, Units2Read, pDest);
};
...
bool CRAIDProcessor::WriteStripeUnit(unsigned long long StripeID, ///identifies the codeword (stripe)
    ...
    unsigned SubarrayID = ErasureSetID / m_Length;
    SymbolID = (SymbolID + ErasureSetID) % m_Length;
    return m_pArray->m_pDisks[SymbolID + SubarrayID * m_Length].WriteData(
        StripeID * m_StripeUnitsPerSymbol + StripeUnitID, Units2Write, pSrc);
};
...
bool CRAIDProcessor::ReadData(unsigned long long StripeID, ///the stripe to be read
    ...
    if (FirstSymbolOffset) {
        ...
        Result &= DecodeDataSubsymbols(StripeID, ErasureSetID, FirstSymbolID, FirstSymbolOffset, Units2Read, pDest,
                                       ThreadID);
...
bool CRAIDProcessor::WriteData(unsigned long long StripeID, ///the stripe to be written
    ...
    if (GetEncodingStrategy(ErasureSetID, StripeUnitID, NumOfUnits)) {
        if (NumOfUnits == m_Dimension * m_StripeUnitsPerSymbol)
            ...
            unsigned char *pBuffer = m_pUpdateBuffer + ThreadID * m_Dimension * m_StripeUnitsPerSymbol *
                                     m_StripeUnitSize;
            if (StripeUnitID) {
            ...
            if (TrailingUnits > 0) {
                ...
                Result &= ReadData(StripeID, StripeUnitID + NumOfUnits, SubarrayID, TrailingUnits,
                                   pBuffer + (StripeUnitID + NumOfUnits) * m_StripeUnitSize, ThreadID);
            };
