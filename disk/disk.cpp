...
CDisk::CDisk(
    const char* pFilename, /// the name of the backend file
...
bool CDisk::Initialize(
  ...
  if ((Header.MagicNumber != MAGICNUMBER) || (Header.HeaderVersion != DISKHEADERVERSION)) {
  ...
  if (Header.Valid) {
    ...
    m_DiskState = dsInvalid; // the disk was not properly initialized before
  }
...
bool CDisk::ReadData(
    unsigned long long BlockID, /// the first block to be read
...
bool CDisk::WriteData(
    unsigned long long BlockID, /// start of the destination area
