...
CDiskArray::CDiskArray(
    ...
    /// the number of disks to be emulated. This must be not less
    /// than the length of the array code implemented by Processor
    /// All extra disks will be ignored
    const DiskConf* pDiskFiles, /// configuration of the emulated disks
    ...
    , m_NumOfStripes(DiskCapacity / (Processor.GetStripeUnitSize() * Processor.GetStripeUnitsPerSymbol()))
    , m_StripeSize(m_UnitsPerStripe * m_StripeUnitSize)
    , m_Locker(NumOfThreads) {
  if (Processor.GetCodeLength() * Processor.GetInterleavingOrder() > m_NumOfDisks) {
    throw Exception(
        "Not enough disks for a given code (minimum %d is required)",
        Processor.GetCodeLength() * Processor.GetInterleavingOrder()
    );
  } else {
    m_NumOfDisks = Processor.GetCodeLength() * Processor.GetInterleavingOrder();
  }

  const void* pCodeConfig;
  ...
  for (unsigned i = 0; i < m_NumOfDisks; i++) {
    if (m_pDisks[i].Initialize(
            pDiskFiles[i].pFileName,
            i,
            m_StripeUnitSize,
...
///@return true on success
bool CDiskArray::Read(
    unsigned long long StripeUnitID, /// the first stripe unit
...
///@return true on success
bool CDiskArray::Write(
    unsigned long long StripeUnitID, /// the first stripe unit
    unsigned long long Units2Write, /// the number of stripe units to be written
    const unsigned char* pSrc,
    /// source buffer. Must have size for Units2Write*m_StripeUnitSize bytes
    size_t ThreadID /// the ID of a calling thread obtained from m_Locker
) {
 ...
 */
long long CDiskArray::read(
    tHandle& fd, /// file description, i.e. current position
