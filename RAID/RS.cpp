...
int GetForneyMultiple(
    unsigned LambdaDegree, // the degree of the erasure locator polynomial
...
CRSProcessor::CRSProcessor(RSParams* pParams)
    : CRAIDProcessor(pParams->CodeDimension + pParams->Redundancy, 1, pParams, sizeof(RSParams))
    , m_Redundancy(pParams->Redundancy)
    , m_pErasureLocatorsPrime(0)
    , m_pSyndromes(0)
    , m_pErasureEvaluator(0)
    , m_pErasureLocators(0)
    , m_pSymbols(0)
    , m_ppSymbols(0)
    , m_pCyclotomicTemp(0) {
...
*/
bool CRSProcessor::Attach(
    CDiskArray* pArray, /// the disk array
    unsigned ConcurrentThreads
    /// the number of concurrent processing threads that will make calls to the processor
) {
...
*/
bool CRSProcessor::IsCorrectable(
  ...
  if (GetNumOfErasures(ErasureSetID) > m_Redundancy) {
    return false;
  }
...
*/
void ComputeSyndrome(
    const GFValue* const* ppData,
    /// pointers to y_j. Each y_j is an array of size UnitSize. If ppData[i]=0, the symbols are assumed to be 0
  ...
  if (ppData[0]) {
    ...
    memset(pSyndromes, 0, UnitSize * (High - Low) * sizeof(GFValue));
  }
...
void GetErasureEvaluator(
    const GFValue* pSyndromes, /// S(x) stored blockwise
...
*/
void Evaluate(
    const GFValue* pPolynomials, /// polynomials stored blockwise
...
*/
bool CRSProcessor::DecodeDataSymbols(
    ...
    /// the number of subsymbols within this symbol to be decoded
    unsigned char* pDest,
    /// destination array. Must have size at least Subsymbols2Decode*m_StripeUnitSize
  ...
  if (NeedsDecoding) {
    ...
    for (unsigned i = 0; i < m_Redundancy; i++) {
      if (IsErased(ErasureSetID, m_Dimension + i)) {
        ...
        if (!ReadStripeUnit(
                ...
                0,
                1,
                pFetchBuffer + (m_Dimension + i) * m_StripeUnitSize
            )) {
          return false;
        }
      };
    ...
    GetErasureEvaluator(
        pSyndrome,
        m_pErasureLocators + ErasureSetID * (m_Redundancy + 1),
        pErasureEvaluator,
        GetNumOfErasures(ErasureSetID),
        m_StripeUnitSize
    );
...
///@return true on success
bool CRSProcessor::EncodeStripe(
    unsigned long long StripeID, /// the stripe to be encoded
    unsigned ErasureSetID, /// identifies the load balancing offset
...
*/
bool CRSProcessor::GetEncodingStrategy(
    unsigned ErasureSetID, /// identifies the load balancing offset
...
This will fetch old values of the symbols to be updated, compute the corresponding syndrome and
   ...
   */
bool CRSProcessor::UpdateInformationSymbols(
...
*/
bool CRSProcessor::CheckCodeword(
  ...
  for (unsigned i = 0; i < m_Dimension; i++) {
    GFValue* pCurSymbol = pFetchBuffer + i * m_StripeUnitSize;
    if (!ReadStripeUnit(StripeID, ErasureSetID, i, 0, 1, pCurSymbol)) {
      return false;
    }
  ...
  for (unsigned i = 0; i < m_Redundancy; i++) {
    ...
    if (!ReadStripeUnit(StripeID, ErasureSetID, m_Dimension + i, 0, 1, pCurSymbol)) {
      return false;
    }
