...
bool CRDPProcessor::Attach(CDiskArray* pArray, unsigned ConcurrentThreads) {
  m_pRDPXORBuffer = AlignedMalloc(ConcurrentThreads * BUFFER_SIZE);
...
bool CRDPProcessor::CheckCodeword(unsigned long long StripeID, unsigned ErasureSetID, size_t ThreadID) {
  ...
  for (long long i = 1; i < m_PrimeNumber; i++) {
    ...
    for (long long k = 0; k < m_StripeUnitsPerSymbol; k++) {
      for (long long j = 0; j < MAX_ERASURES; j++) {
        ...
        XOR(pXORBuffer[j] + Diag * m_StripeUnitSize, pReadBuffer + k * m_StripeUnitSize, m_StripeUnitSize);
      }
    }
  }
  ...
  if (!Result) {
    ...
    return std::ranges::all_of(pXORBuffer, [this](auto res) {
      return std::accumulate(
                 res,
                 res + m_StripeUnitSize * m_StripeUnitsPerSymbol,
                 0,
...
bool CRDPProcessor::EncodeStripe(
  ...
  for (unsigned i = 1; i < m_Dimension; i++, pData += m_StripeUnitSize * m_StripeUnitsPerSymbol) {
    ...
    for (long long j = 0; j < MAX_ERASURES; j++) {
      ...
      for (long long k = 0; k < m_StripeUnitsPerSymbol; k++) {
        ...
        XOR(pXORBuffer[j] + Diag * m_StripeUnitSize, pData + k * m_StripeUnitSize, m_StripeUnitSize);
      }
    }
  }
...
bool CRDPProcessor::DecodeDataSubsymbols(
    unsigned long long StripeID,
    unsigned ErasureSetID,
    unsigned SymbolID,
    ...
    if (i >= m_PrimeNumber) {
      ...
      for (long long j = 0; j < MAX_ERASURES; j++) {
        ...
        for (long long k = 0; k < m_StripeUnitsPerSymbol; k++) {
          ...
          XOR(pXORBuffer[j] + Diag * m_StripeUnitSize, pBuffer + k * m_StripeUnitSize, m_StripeUnitSize);
        }
      }
    }
  }
  ...
  while (diag != m_PrimeNumber - 1) {
    if (RecoverResults[diag_restore]) {
      memcpy(
          ...
          pXORBuffer[diag_step] + diag * m_StripeUnitSize,
          m_StripeUnitSize
      );
    }
    ...
    XOR(pXORBuffer[0] + row * m_StripeUnitSize, pXORBuffer[diag_step] + diag * m_StripeUnitSize, m_StripeUnitSize);
    if (RecoverResults[horizontal_restore]) {
      memcpy(
          RecoverResults[horizontal_restore] + row * m_StripeUnitSize,
          pXORBuffer[0] + row * m_StripeUnitSize,
          m_StripeUnitSize
      );
    }
    ...
    if (diag != m_PrimeNumber - 1) {
      XOR(pXORBuffer[diag_step] + diag * m_StripeUnitSize, pXORBuffer[0] + row * m_StripeUnitSize, m_StripeUnitSize);
    }
...
bool CRDPProcessor::DecodeTwoErasures(
    ...
    std::array<int, 2> ErasurePosition = {-1, -1}
) {
  bool Result = true;
...
bool CRDPProcessor::DecodeThreeErasures(
  ...
  if (ErasurePosition[1] >= m_PrimeNumber) {
    if (std::none_of(ErasurePosition.begin() + 1, ErasurePosition.end(), [&](int i) {
      ...
      return DecodeOneErasure(StripeID, ErasureSetID, SymbolID, Symbols2Decode, pDest, ThreadID, ErasurePosition[0]);
    } else {
      pXORBuffer[0] = (ErasurePosition[0] >= SymbolID) && (ErasurePosition[0] < SymbolID + Symbols2Decode)
                        ? pDest + (ErasurePosition[0] - SymbolID) * m_StripeUnitSize * m_StripeUnitsPerSymbol
                        : m_pRDPXORBuffer + ThreadID * BLOCKS_PER_BUFFER * BLOCK_SIZE;
      ...
      for (long long i = 1; i < MAX_ERASURES; i++) {
        ...
        for (long long j = 0; j < m_StripeUnitsPerSymbol; j++) {
          ...
          XOR(pXORBuffer[i] + Diag * m_StripeUnitSize, pXORBuffer[0] + j * m_StripeUnitSize, m_StripeUnitSize);
        }
      }
      return Result;
    }
    ...
    if (prod[mod(i - (l + r), m_PrimeNumber)] ^ Missing) {
      for (long long j = 0; j < m_StripeUnitsPerSymbol; ++j) {
        XOR(Recover + j * m_StripeUnitSize,
            ...
            m_StripeUnitSize);
        XOR(Recover + j * m_StripeUnitSize, pXORBuffer[0] + (m_PrimeNumber - 1 - i) * m_StripeUnitSize, m_StripeUnitSize
        );
      }
    }
  }
    ...
    if ((prod[mod(i - l, m_PrimeNumber)] ^ prod[mod(i - r, m_PrimeNumber)]) ^ Missing) {
      for (long long j = 0; j < m_StripeUnitsPerSymbol; ++j) {
        XOR(Recover + j * m_StripeUnitSize,
            ...
            m_StripeUnitSize);
        XOR(Recover + j * m_StripeUnitSize, pXORBuffer[1] + (m_PrimeNumber - 1 - i) * m_StripeUnitSize, m_StripeUnitSize
        );
      }
    }
  }
    ...
    if (prod[i]) {
      for (long long j = 0; j < m_StripeUnitsPerSymbol; ++j) {
        XOR(Recover + j * m_StripeUnitSize,
            ...
            m_StripeUnitSize);
        XOR(Recover + j * m_StripeUnitSize, pXORBuffer[2] + (m_PrimeNumber - 1 - i) * m_StripeUnitSize, m_StripeUnitSize
        );
      }
    }
  }
...
bool CRDPProcessor::DecodeDataSymbols(
    unsigned long long StripeID,
    unsigned ErasureSetID,
    unsigned SymbolID,
    unsigned Symbols2Decode,
  ...
  const unsigned NumberOfErasures = GetNumOfErasures(ErasureSetID);
  if (std::ranges::none_of(std::views::iota(SymbolID, SymbolID + Symbols2Decode), [&](unsigned i) {
...
bool CRDPProcessor::GetEncodingStrategy(unsigned ErasureSetID, unsigned StripeUnitID, unsigned Subsymbols2Encode) {
  return std::ranges::count_if(
             std::views::iota(
                 StripeUnitID / m_StripeUnitsPerSymbol,
                 (StripeUnitID + Subsymbols2Encode - 1) / m_StripeUnitsPerSymbol + 1
             ),
             [&](unsigned i) { return IsErased(ErasureSetID, i); }
         ) != 0;
}

bool CRDPProcessor::UpdateInformationSymbols(
    unsigned long long StripeID,
    unsigned ErasureSetID,
  ...
  if (std::ranges::all_of(std::views::iota(0u, MAX_ERASURES), [&](unsigned i) {
    ...
    long long Units2Write = std::min(
        (m_StripeUnitsPerSymbol - CurrentSubsymbol) % m_StripeUnitsPerSymbol,
        static_cast<long long>(Units2Update)
    );
    if (Units2Write != 0) {
      Result &= WriteStripeUnit(StripeID, ErasureSetID, CurrentSymbol, CurrentSubsymbol, Units2Write, pData);
      CurrentSymbol++;
      pData += Units2Write * m_StripeUnitSize;
      Units2Update -= Units2Write;
    }
    for (unsigned i = 0; i < Units2Update / m_StripeUnitsPerSymbol;
         ++i, pData += m_StripeUnitSize * m_StripeUnitsPerSymbol, CurrentSymbol++) {
      Result &= WriteStripeUnit(StripeID, ErasureSetID, CurrentSymbol, 0, m_StripeUnitsPerSymbol, pData);
    }
    Units2Update %= m_StripeUnitsPerSymbol;
    if (Units2Update != 0) {
      Result &= WriteStripeUnit(StripeID, ErasureSetID, CurrentSymbol, 0, Units2Update, pData);
    }
  } else {
    std::array<unsigned char*, MAX_ERASURES> pXORBuffer{};
    unsigned char* pReadBuffer = m_pRDPXORBuffer + (MAX_ERASURES + ThreadID * BLOCKS_PER_BUFFER) * BLOCK_SIZE;
    ...
    if (Units2Write != 0) {
      Result &= ReadStripeUnit(StripeID, ErasureSetID, CurrentSymbol, CurrentSubsymbol, Units2Write, pReadBuffer);
      XOR(pReadBuffer, pData, Units2Write * m_StripeUnitSize);
      if (pXORBuffer[0]) {
        XOR(pXORBuffer[0] + CurrentSubsymbol * m_StripeUnitSize, pReadBuffer, Units2Write * m_StripeUnitSize);
      }
      for (long long i = 0; i < Units2Write; ++i) {
        for (long long j = 1; j < MAX_ERASURES; ++j) {
          if (!pXORBuffer[j]) {
            continue;
          }
    ...
    for (unsigned i = 0; i < Units2Update / m_StripeUnitsPerSymbol;
         ++i, pData += m_StripeUnitSize * m_StripeUnitsPerSymbol, CurrentSymbol++) {
      Result &= ReadStripeUnit(StripeID, ErasureSetID, CurrentSymbol, 0, m_StripeUnitsPerSymbol, pReadBuffer);
      XOR(pReadBuffer, pData, m_StripeUnitsPerSymbol * m_StripeUnitSize);
      if (pXORBuffer[0]) {
        XOR(pXORBuffer[0], pReadBuffer, m_StripeUnitsPerSymbol * m_StripeUnitSize);
      }
      for (long long j = 0; j < m_StripeUnitsPerSymbol; ++j) {
        for (long long k = 1; k < MAX_ERASURES; ++k) {
          if (!pXORBuffer[k]) {
            continue;
          }
    ...
    Units2Update %= m_StripeUnitsPerSymbol;
    if (Units2Update != 0) {
      Result &= ReadStripeUnit(StripeID, ErasureSetID, CurrentSymbol, 0, Units2Update, pReadBuffer);
      XOR(pReadBuffer, pData, Units2Update * m_StripeUnitSize);
      if (pXORBuffer[0]) {
        XOR(pXORBuffer[0], pReadBuffer, Units2Update * m_StripeUnitSize);
      }
      for (long long i = 0; i < Units2Update; ++i) {
        for (long long j = 1; j < MAX_ERASURES; ++j) {
          if (!pXORBuffer[j]) {
            continue;
          }
