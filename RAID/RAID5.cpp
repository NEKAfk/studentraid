...
CRAID5Processor::CRAID5Processor(RAID5Params* P /// the configuration file
    ...
    : CRAIDProcessor(P->CodeDimension + 1, 1, P, sizeof(*P))
    , m_pXORBuffer(0) {
 ...
 */
bool CRAID5Processor::DecodeDataSymbols(
    ...
    /// the number of symbols within this symbol to be decoded
    unsigned char* pDest,
    /// destination array. Must have size at least Subsymbols2Decode*m_StripeUnitSize
    size_t ThreadID /// the ID of the calling thread
) {
  bool Result = true;
  if ((GetNumOfErasures(ErasureSetID) == 0) || (GetErasedPosition(ErasureSetID, 0) < (int) SymbolID) ||
      (GetErasedPosition(ErasureSetID, 0) >= int(SymbolID + Symbols2Decode))) {
    // read the data as is
 ...
 */
bool CRAID5Processor::EncodeStripe(
 ...
 */
bool CRAID5Processor::UpdateInformationSymbols(
 ...
 * @return true on success
 */
bool CRAID5Processor::CheckCodeword(
